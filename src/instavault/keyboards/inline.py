...
def order_keyboard_full() -> InlineKeyboardMarkup:
    ...
    for pkg_key, pkg_data in PACKAGES.items():
        ...
        builder.button(text=btn_text, callback_data=f"order_pkg_{pkg_key}")

    # Add back button
    builder.button(text="🏠 Back to Dashboard", callback_data="go_dashboard")
...
def order_history_keyboard(
    has_prev: bool = False, has_next: bool = False, page: int = 0
    ...
    if has_prev:
        nav_row.append(
            InlineKeyboardButton(
                text="⬅️ Prev", callback_data=f"order_history_page:{page - 1}"
            )
        )
    if has_next:
        nav_row.append(
            InlineKeyboardButton(
                text="Next ➡️", callback_data=f"order_history_page:{page + 1}"
            )
        )
    if nav_row:
        rows.append(nav_row)
    ...
    rows.append(
        [
            InlineKeyboardButton(
                text="🏠 Back to Dashboard", callback_data="go_dashboard"
            )
        ]
    )
