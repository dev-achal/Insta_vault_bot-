...
async def cmd_start(message: Message, state: FSMContext) -> None:
    ...
    logger.info(
        "New user started onboarding: %s (%s) ref=%s", user_id, first_name, ref_code
    )
...
async def cb_beat_3(query: CallbackQuery, state: FSMContext) -> None:
    ...
    try:
        ...
        await query.message.edit_text(
            "⚠️ Account creation failed due to a server error. Please try again later."
        )
        return
...
async def cb_nav_dashboard(query: CallbackQuery) -> None:
    ...
    if query.message and user:
        await show_dashboard(
            user.id, user.first_name or "Member", query.message, edit=False, query=query
        )
