...
def user_manage_keyboard(target_id: str, is_banned: bool) -> InlineKeyboardMarkup:
    ...
    action_button = (
        InlineKeyboardButton(
            text="✅ Unban User", callback_data=f"admin_unban_user:{target_id}"
        )
        if is_banned
        else InlineKeyboardButton(
            text="🚫 Ban User", callback_data=f"admin_ban_user:{target_id}"
        )
    )
...
async def handle_user_id_input(message: Message, state: FSMContext) -> None:
    ...
    status_text = (
        "🔴 <b>BANNED (Suspended)</b>" if banned else "🟢 <b>ACTIVE (Normal)</b>"
    )
...
async def cb_ban_user(query: CallbackQuery) -> None:
    ...
    try:
        ...
        await query.message.edit_text(
            text, reply_markup=user_manage_keyboard(target_id, is_banned=True)
        )
    except Exception as err:
        logger.error("Failed to ban user %s: %s", target_id, err)
        await query.answer(
            "⚠️ Failed to update ban status in database.", show_alert=True
        )
...
async def cb_unban_user(query: CallbackQuery) -> None:
    ...
    try:
        ...
        await query.message.edit_text(
            text, reply_markup=user_manage_keyboard(target_id, is_banned=False)
        )
    except Exception as err:
        logger.error("Failed to unban user %s: %s", target_id, err)
        await query.answer(
            "⚠️ Failed to update unban status in database.", show_alert=True
        )
