...
def broadcast_confirm_keyboard() -> InlineKeyboardMarkup:
    ...
    return InlineKeyboardMarkup(
        inline_keyboard=[
            [
                InlineKeyboardButton(
                    text="🚀 Confirm & Broadcast Now", callback_data="confirm_broadcast"
                ),
            ],
            [
                InlineKeyboardButton(
...
async def cb_cron_select_time(query: CallbackQuery, state: FSMContext) -> None:
    ...
    if action == "cancel":
        ...
        await query.message.edit_text(
            "❌ Scheduled Broadcast setup cancelled.",
...
async def handle_cron_custom_minutes_input(message: Message, state: FSMContext) -> None:
    ...
    if not text.isdigit() or int(text) <= 0:
        await message.answer(
            "⚠️ Invalid number of minutes. Please send a valid positive number (e.g. <code>30</code>):"
        )
        return
...
@router.callback_query(
    CronBroadcastState.confirm_send, F.data == "cancel_cron_broadcast"
)
...
@router.callback_query(
    CronBroadcastState.confirm_send, F.data == "confirm_cron_broadcast"
)
async def cb_confirm_cron_broadcast(query: CallbackQuery, state: FSMContext) -> None:
    ...
    if not content_chat_id or not content_message_id:
        await query.message.edit_text(
            "❌ Error: Broadcast payload lost. Please try again."
        )
        return
...
async def run_scheduled_broadcast_task(
    ...
    logger.info(
        "⏰ Scheduled broadcast queued to fire in %d seconds (%s)",
        delay_seconds,
        fire_time_str,
    )
    ...
    for target_uid in user_ids:
        ...
        while not delivered:
            try:
                ...
                logger.warning(
                    "Rate limit hit during scheduled broadcast. Sleeping for %s seconds.",
                    e.retry_after,
                )
                ...
                logger.error(
                    "Failed to deliver scheduled broadcast to %s: %s", target_uid, e
                )
                failed_count += 1
                delivered = True
            except Exception as e:
                logger.error(
                    "Unexpected error delivering scheduled broadcast to %s: %s",
                    target_uid,
                    e,
                )
                failed_count += 1
    ...
    try:
        ...
        logger.error(
            "Failed to send scheduled broadcast completion report to admin %d: %s",
            admin_id,
            err,
        )
...
async def cmd_cancel_all_broadcast(message: Message, state: FSMContext) -> None:
    ...
    await state.clear()
    await message.answer(
        "❌ All Users Broadcast cancelled.", reply_markup=broadcast_menu_keyboard()
    )
    ...
    for target_uid in user_ids:
        ...
        while not delivered:
            try:
                ...
                logger.warning(
                    "Telegram FloodWait hit during broadcast! Sleeping for %s seconds",
                    retry_err.retry_after,
                )
                await asyncio.sleep(retry_err.retry_after + 0.5)
            except (TelegramBadRequest, TelegramAPIError) as err:
                logger.error(
                    "Failed to deliver broadcast to user %s: %s", target_uid, err
                )
                failed_count += 1
                delivered = True
            except Exception as err:
...
async def cmd_cancel_single_broadcast(message: Message, state: FSMContext) -> None:
    ...
    await state.clear()
    await message.answer(
        "❌ Single User Direct Broadcast cancelled.",
        reply_markup=broadcast_menu_keyboard(),
    )
...
@router.callback_query(
    SingleBroadcastState.confirm_send, F.data == "cancel_single_broadcast"
)
async def cb_cancel_single_broadcast_confirm(
...
@router.callback_query(
    SingleBroadcastState.confirm_send, F.data == "confirm_single_broadcast"
)
async def cb_confirm_single_broadcast(query: CallbackQuery, state: FSMContext) -> None:
    ...
    if not target_uid or not from_chat_id or not message_id:
        await query.message.edit_text(
            "❌ Error: Message payload lost. Please try again."
        )
        return
