...
async def cache_user_data(
    user_id: int | str, data: dict[str, Any], ttl_seconds: int = 86400
) -> None:
    ...
    for attempt in range(1, max_attempts + 1):
        try:
            ...
            if attempt < max_attempts:
                logger.warning(
                    "Failed to invalidate cache for %s on attempt %d: %s. Retrying...",
                    user_id,
                    attempt,
                    e,
                )
                ...
                logger.error(
                    "Failed to invalidate cache for %s after %d attempts: %s",
                    user_id,
                    max_attempts,
                    e,
                )
