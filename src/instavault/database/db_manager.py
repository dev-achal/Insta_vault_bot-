...
async def create_user_transactional(
    ...
    if result:
        ...
        from instavault.database.redis_manager import record_new_account
...
async def get_leaderboard(limit: int = 10) -> list[dict[str, Any]]:
    ...
    try:
        ...
        return (
            []
        )  # Graceful degradation (shows empty leaderboard instead of crashing bot)
    ...
    async def _run_in_tx(tx) -> str:
        ...
        if user_data.get("last_order_nonce") == nonce:
            raise DuplicateOrderError(
                f"Duplicate order: nonce {nonce} already processed."
            )
...
async def get_banned_user_ids() -> set[str]:
    ...
    try:
        ...
        query = (
            db.collection(USERS_COL)
            .where(filter=FieldFilter("is_banned", "==", True))
            .select([])
        )
        async for doc in query.stream():
...
async def get_today_new_accounts_count_firestore() -> int:
    ...
    try:
        ...
        query_legacy = (
            db.collection(USERS_COL)
            .where(filter=FieldFilter("join_date", ">=", today_start))
            .count()
        )
...
async def get_today_new_accounts_count() -> int:
    ...
    """
    try:
        from instavault.database.redis_manager import get_today_new_accounts_count_redis
...
async def complete_shortener_task(user_id: int, reward: int) -> None:
    ...
    from instavault.database.redis_manager import record_shortener_completion
